...
export(PackedScene) var missile_object:PackedScene
export(PackedScene) var ball_object:PackedScene
export(PackedScene) var laser_object:PackedScene
...
func shoot_ball():
	...
	new_ball.apply_impulse(Vector3(0, 0, 0), -camera.global_transform.basis.z.normalized() * 150)
...
#			if grabbed_object is MassPoint:
#				grabbed_object._shatter(camera.global_transform.basis.z.normalized() * 5)
#			else:
